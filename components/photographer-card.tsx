...
export function PhotographerCard({ photographer }: PhotographerCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Default State */}
      <div
        className={`absolute inset-0 transition-opacity duration-300 ${isHovered ? "opacity-0" : "opacity-100"} flex flex-col p-7 bg-white text-gray-900 rounded-2xl`}
      >
        ...
        <div className="flex-grow flex flex-col justify-between">
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-4">
            {photographer.mainStyle && (
              <Badge className={`${getStyleColor(photographer.mainStyle)} font-medium px-3 py-1 rounded-full`}>
                {photographer.mainStyle}
              </Badge>
            )}
            {filteredAdditionalStyles.slice(0, 2).map((style, index) => (
              <Badge
                key={index}
                variant="outline"
                className="text-xs rounded-full border-gray-200 text-gray-700 bg-gray-100"
              >
                {style}
              </Badge>
            ))}
      ...
      {/* Hover State */}
      <div
        className={`absolute inset-0 transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"} flex flex-col overflow-hidden rounded-2xl`}
      >
        <div className="flex flex-col h-full">
          <div className="flex-grow relative overflow-hidden">
            <img
              src={featuredImages[0] || portfolioImage || "/placeholder.svg"}
          ...
          <div className="flex-grow relative overflow-hidden">
            <img
              src={featuredImages[1] || portfolioImage || "/placeholder.svg"}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end bg-gradient-to-t from-black/80 via-black/30 to-transparent text-white p-6">
          ...
          <Button
            onClick={() => router.push(`/book/${photographer.id}`)}
            className="w-3/4 max-w-sm border border-white text-white bg-blue-600 hover:bg-white hover:text-blue-600 transition-all duration-200 rounded-lg shadow-lg font-semibold py-2 text-lg"
          >
            View Portfolio & Book
          </Button>
        </div>
